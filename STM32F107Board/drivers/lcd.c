...
Note:touch_spi.c 中定义了SSD1289  和 ili9325 的触摸屏位置 配置宏，需要根据使用的LCD模块进行选择，不能自动 选择。
Version:         1.0 
History:         
...
//竖屏幕 不需要  _ILI_REVERSE_DIRECTION_
//横屏幕 需要  _ILI_REVERSE_DIRECTION_
...
static u16 DeviceIdCode;  //设置一个静态变量保存LCD的ID
	...
	/** 开启内部振荡器 */
 	lcd_write_reg(0x0000,0x0001);		
        ...
        else if(DeviceIdCode==0x9325)//ili9325 
        {
          ...
          lcd_write_reg(0x0003,0x1020 );  // set GRAM write direction and BGR=1. 
...
void rt_hw_lcd_set_pixel(const char* c, int x, int y)
{
    rt_uint16_t p;
    p =  *(uint16_t *)c;
    lcd_set_point((uint16_t)x, (uint16_t)y, p); 
}

/* 读取某个坐标上像素点，按照RGB格式返回*/
void rt_hw_lcd_get_pixel(char* c, int x, int y)
{

  *c = rtgui_color_from_565(lcd_get_point((uint16_t)x, (uint16_t)y));
}
...
#if defined(_ILI_REVERSE_DIRECTION_)
    /* [5:4]-ID~ID0 [3]-AM- 1垂直 0-水平 */
    lcd_write_reg(0x0003,0x1010 | (1<<3)); // AM=1 vline
#else
    /* [5:4]-ID~ID0 [3]-AM- 1垂直 0-水平 */
    lcd_write_reg(0x0003,0x1010 | (0<<3)); // AM=0 hline
#endif
    }
...
void rt_hw_lcd_draw_vline(const char* c, int x, int y1, int y2)
{
    ...
    lcd_set_cursor(x, y1);
    lcd_ram_prepare(); /* Prepare to write GRAM */
    while (y1 < y2)
    {
        lcd_write_ram(p);
        y1++;
