...
Description:    1.adc驱动,带数字滤波  
                2.使用片内的温度传感器测量温度,并通过串口输出温度值  
                3. B0 B1 C0 使用ADC1 通道8、9、10 
Version:         2.0 
History:         
      <author>  <time>   <version >   <desc> 
      Sundm    13/03/30     1.0     文件创建   
      Sundm    15/10/30     1.0     添加数字滤波部分Filter   
      Sundm    16/04/30     1.0     添加多通道AD  
...
uint16_t  ADCConvertedTempValue[FilterLen];
uint16_t  ADCConvertedValue[FilterLen];

uint16_t ADCConvertedMulValue[Sample_Num][Channel_Num];
...
/*******************************************************************************
* Function Name  : DMA_TempConfig
* Description    : 温度测量 DMA 配置 内部函数
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void DMA_TempConfig(void) 
{ 
  DMA_InitTypeDef DMA_InitStructure;     
  /* Enable DMA1 clock */
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);
  ...
  DMA_DeInit(DMA1_Channel1);
  DMA_InitStructure.DMA_PeripheralBaseAddr = ADC1_DR_Address;    //DMA对应的外设基地址
  DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADCConvertedTempValue;   //内存存储基地址
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;	//DMA的转换模式为SRC模式，由外设搬移到内存
  DMA_InitStructure.DMA_BufferSize = FilterLen;		   //DMA缓存大小，1个,单位为DMA_MemoryDataSize
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;	//接收一次数据后，设备地址禁止后移
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;	//关闭接收一次数据后，目标内存地址增加
...
/*******************************************************************************
* Function Name  : DMA_Mul_Config
* Description    : ADC DMA 3通道配置 内部函数
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void DMA_Mul_Config(void) 
{ 
  DMA_InitTypeDef DMA_InitStructure;     
  /* Enable DMA1 clock */
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);
  ...
  DMA_DeInit(DMA1_Channel1);
  DMA_InitStructure.DMA_PeripheralBaseAddr = ADC1_DR_Address;    //DMA对应的外设基地址
  DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADCConvertedMulValue;   //内存存储基地址
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;	//DMA的转换模式为SRC模式，由外设搬移到内存
...
void ADC_Config(void)
{
  ...
  /* ADC1 regular channel 8 configuration ADC通道组， 第8个通道 采样顺序1，转换时间 */ 
  /* PB0作为AI 用ADC_Channel_8
  ADC_RegularChannelConfig(ADC1, ADC_Channel_8, 1, ADC_SampleTime_55Cycles5);*/
  
  /* ADC1 regular channel 9 configuration ADC通道组， 第9个通道 采样顺序1，转换时间 */ 
...
/*******************************************************************************
* Function Name  : ADC_Mul_Config
* Description    : ADC 多通道转换 采用DMA方式　初始化
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
void getadcvalue(void)
{
  ...
  adcvalue = DigitFilter(ADCConvertedValue,FilterLen);
...
{ 
 uint16_t adctempvalue = 0; 
 
  adctempvalue = DigitFilter(ADCConvertedTempValue,FilterLen);  
  adctempvalue = (1430 - adctempvalue * 3300/4096)/4.3 + 25;//转换为温度值 
...
void getmuladcvalue(void)
{
  ...
  for(i=0;i<Channel_Num;i++ )
  {
    ...
    {
      adcvaluetemp[j] = ADCConvertedMulValue[j][i];
    }
