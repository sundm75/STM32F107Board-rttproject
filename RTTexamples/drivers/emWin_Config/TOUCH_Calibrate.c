...
*/
/*******************************2012-2014, NJUT, Edu.************************** 
FileName: touch_calibrate.c 
Author:  孙冬梅       Version :  1.0        Date: 2014.06.04
Description:    触摸屏校准   
    按照以下步骤运行
    1.startgui()：初始化GUI，清屏
    2.DynamicCali() ：触摸屏校准函数
    3.StopCali()：退出校准程序
...
*       MainTask
*/
void Entry_TouchCali(void* parameter) 
{
...
void StopCali(void)
{
  rt_thread_t thread;
  rt_enter_critical();
  thread = rt_thread_find("TouchCali");
  if( thread != RT_NULL)
...
#include <finsh.h>
FINSH_FUNCTION_EXPORT(DynamicCali, startup DynamicCali);
FINSH_FUNCTION_EXPORT(StopCali, startup StopCali);
#endif
