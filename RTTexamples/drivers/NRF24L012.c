...
Description:    NRF24L01-2驱动   使用SPI3   
注：本程序与NRF24L01.C完全一致，在同一块开发板上，配对2个RF模块使用
Version:         1.0 
History:         
...
const rt_uint8_t static  TX_ADDRESS[TX_ADR_WIDTH]={0x73,0x75,0x6E,0x64,0x6D}; //发送地址
...
static rt_uint8_t NRF24L01_Read_Buf(rt_uint8_t regaddr,rt_uint8_t *pBuf,rt_uint8_t datalen)
{
  ...
  for(rt_uint8_t_ctr=0;rt_uint8_t_ctr<datalen;rt_uint8_t_ctr++)pBuf[rt_uint8_t_ctr]=SPI3_ReadWriteByte(0XFF);//读出数据
  Set_NRF24L01_CSN_2;                     //关闭SPI传输
...
static rt_uint8_t NRF24L01_Write_Buf(rt_uint8_t regaddr, rt_uint8_t *pBuf, rt_uint8_t datalen)
{
  ...
  Clr_NRF24L01_CSN_2;                                    //使能SPI传输
  status = SPI3_ReadWriteByte(regaddr);                //发送寄存器值(位置),并读取状态值
  for(rt_uint8_t_ctr=0; rt_uint8_t_ctr<datalen; rt_uint8_t_ctr++)SPI3_ReadWriteByte(*pBuf++); //写入数据	 
  Set_NRF24L01_CSN_2;                                    //关闭SPI传输
  return status;                                       //返回读到的状态值
}				   
...
static rt_err_t NRF24L01_TxPacket(rt_uint8_t *txbuf)
{
  ...
  Set_NRF24L01_CE_2;                                     //启动发送	   
  while(READ_NRF24L01_IRQ_2!=0);                              //等待发送完成
...
{														 
  ...
  Set_NRF24L01_CE_2;                                  //CE为高,10us后启动发送
}		  
...
static void NRF24L01_Init(void)
{
  ...
  /*config CE CSN*/
  RCC_APB2PeriphClockCmd(RCC_NRF24L01_CE_2, ENABLE);           //使能GPIO的时钟
  GPIO_InitStructure.GPIO_Pin = NRF24L01_CE_PIN_2;              //NRF24L01 模块片选信号
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;           //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIO_NRF24L01_CE_2, &GPIO_InitStructure);

  RCC_APB2PeriphClockCmd(RCC_NRF24L01_CSN_2, ENABLE);          //使能GPIO的时钟
  GPIO_InitStructure.GPIO_Pin = NRF24L01_CSN_PIN_2;      
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;           //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIO_NRF24L01_CSN_2, &GPIO_InitStructure);

  Set_NRF24L01_CE_2;                                           //初始化时先拉高
  Set_NRF24L01_CSN_2;                                   //初始化时先拉高
  ...
  SPI3_Init();                                       //初始化SPI
  Clr_NRF24L01_CE_2; 	                               //使能24L01
  Set_NRF24L01_CSN_2;                                  //SPI片选取消
}
