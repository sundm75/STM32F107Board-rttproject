...
static void _AddData_Sine(GRAPH_DATA_Handle hData, int DataID) {
  ...
  case 0:
    Multi = 70;
    Step  = 3;
    SinHQ = adcvalue[0]*100;
    break;
  case 1:
    Multi = 50;
    Step  = 1;
     SinHQ = adcvalue[1]*100;
    break;
  case 2:
    Multi = 30;
    Step  = 7;
     SinHQ = adcvalue[2]*100;
    break;
  default:
    return;
  }
  
  GRAPH_DATA_YT_AddValue(hData, ((SinHQ ) >> 12) + _DataAdjust);
}
...
GRAPH_WAVE _aWave[] = {
  { "Heartbeat",        // Name
    157,                // Vertical scale offset in relation to GRAPH_DIV 垂直刻度偏移 
    152,                // Vertical data  offset in relation to GRAPH_DIV 垂直数据偏移 
    21,                 // Vertical grid  offset in relation to GRAPH_DIV 垂直栅格偏移 
...
static void _ShowGraph(GRAPH_Handle hGraph, GRAPH_DATA_Handle hData[], int DataCount, void (* pfAddData)(GRAPH_DATA_Handle hData, int DataID)) {
  ...
  while (Count < Data_xSize) {
    for (i = 0; i < DataCount; i++) {
      pfAddData(hData[i], i);//用这个函数添加要显示的数据
    }
    Count++;
  }
  //
  ...
  do {
    ...
    char str[20];
    sprintf(str,"RES = %d\r\nAI1 = %d\r\nAI2 = %d\r\n",adcvalue[0],adcvalue[2],adcvalue[1]);
    GUIDEMO_ShowInfo((char const*)str);
  } while ( (NextState == 0));
  for (i = 0; i < DataCount; i++) {
...
void GUIDEMO_Graph(void) {
  GUIDEMO_ShowIntro("Test System & Draw a graph",
                    "NJTECH\n"
                    "for drawing graph data");//显示控制条进度
  GUIDEMO_ShowInfoWin();
  _GraphDemo();
  GUIDEMO_NotifyStartNext();
}
